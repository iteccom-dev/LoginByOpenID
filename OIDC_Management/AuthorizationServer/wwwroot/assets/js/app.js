(function () {
			...
			if (navbarNav) {
				...
				if (a) {
					...
					if (parentCollapseDiv) {
						...
						var closestCollapse = parentCollapseDiv.parentElement.closest('.collapse.menu-dropdown');
						if (closestCollapse) {
							closestCollapse.classList.add("show");
							if (closestCollapse.previousElementSibling) {
								closestCollapse.previousElementSibling.classList.add("active");
							}
							var grandParentCollapse = closestCollapse.parentElement.parentElement.parentElement.parentElement.closest(".collapse.menu-dropdown");
							if (grandParentCollapse) {
								...
								if (grandParentCollapse.previousElementSibling) {
									...
									if ((document.documentElement.getAttribute("data-layout") == "horizontal") && grandParentCollapse.parentElement.parentElement.parentElement.closest(".collapse")) {
										grandParentCollapse.parentElement.parentElement.parentElement.closest(".collapse").previousElementSibling.classList.add("active");
									}
								}
							}
						}
	...
	function hideShowLayoutOptions(dataLayout) {
		if (dataLayout == "vertical") {
			...
			var themeSettingsOffcanvas = document.getElementById("theme-settings-offcanvas");
			if (themeSettingsOffcanvas) {
				...
				var sidebarView = document.getElementById("sidebar-view");
				if (sidebarView) {
				...
				var layoutPosition = document.getElementById("layout-position");
				if (layoutPosition) {
					layoutPosition.style.display = "block";
				}

				var layoutWidth = document.getElementById("layout-width");
				if (layoutWidth) {
					layoutWidth.style.display = "block";
				}

				var sidebarVisibility = document.getElementById("sidebar-visibility");
    ...
    function initAvatarUpload() {
		...
		if (avatarFileUpload) {
			...
			avatarFileUpload.addEventListener("change", function () {
				var avatarContainer = avatarFileUpload.closest(".avatar-container");
				var avatarPreview = avatarContainer.querySelector(".avatar-preview");
				var avatarUrl = avatarContainer.querySelector(".avatar-file-url");
				var file = avatarFileUpload.files[0];
				var reader = new FileReader();
				reader.addEventListener("load", function () {
					avatarPreview.src = reader.result;
					if (avatarUrl) {
						avatarUrl.value = avatarFileUpload.value;
					}
			...
			//var avatarRemove = avatarFileUpload.closest(".avatar-container").querySelector(".avatar-remove");
			//if (avatarRemove) {
